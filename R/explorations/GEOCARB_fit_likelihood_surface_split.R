##==============================================================================
## GEOCARB_fit_likelihood_surface_split.R
##
...
likelihood_fit <- vector('list', n_shard)

for (ss in 1:n_shard) {

    likelihood_fit[[ss]] <- vector('list', n_time)

    for (tt in 1:n_time) {
        idx <- which(time[tt]-data_calib$age < 10 & time[tt]-data_calib$age >= 0 &
                     data_calib$co2 >= co2_split[ss,1] & data_calib$co2 < co2_split[ss,2])
        if (length(idx) > 0) {
...
f_fit <- likelihood_fit[[1]][[34]](co2)
plot(co2,f_fit, xlab='CO2 (ppmv)', ylab='density', type='l', lwd=2, xlim=c(0,5000))
}
##==============================================================================
...
##==================================
idx_data <- vector('list', n_shard)
for (ss in 1:n_shard) {
  for (tt in 1:n_time) {
    if(!is.null(likelihood_fit[[ss]][[tt]])) {idx_data[[ss]] <- c(idx_data[[ss]],tt)}
  }
}
##==============================================================================
...
##   Just need to feed in likelihood_fit[[ss]] instead, as the only modification
##==============================================================================
