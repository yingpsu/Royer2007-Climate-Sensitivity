...
##
## Fit gamma distributions at each time point, assuming the +sigma1/-sigma2
...
print(c(qgamma(p=0.16, shape=parameters.co2$shape[i], scale=parameters.co2$scale[i]),qgamma(p=0.84, shape=parameters.co2$shape[i], scale=parameters.co2$scale[i])))
...
dat.co2 <- cbind(dat,parameters.co2$shape, parameters.co2$scale)
