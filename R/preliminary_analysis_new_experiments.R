...
saveRDS(chains, file="chains_analysis_tmp.rds")

# start back up again
chains <- readRDS("chains_analysis_tmp.rds")
...
pdf(file='../figures/boxplot_ess.pdf', width=7, height=5, colormodel="cmyk")
offset <- 0.1
yhgt <- 0.05
experiment <- rownames(ess$ess)[1]
plot(ess$ess[experiment,"0.5"], yhgt, xlim=c(0,11), ylim=c(0, 1), pch=16, xaxs='i', yaxs='i', yaxt='n', ylab='', xlab='')
grid()
mtext("deltaT2X [deg C]", side=1, line=2.2, cex=0.9)
...
# TODO - 4 row x 2 col figure of each experiment relative to the original PR2011
# Include the data set for each

pdf('../figures/model_experiments_vs_pr2011.pdf',width=9,height=6,colormodel='cmyk')
par(mfrow=c(3,3))
par(cex = 0.75)
...
for (ee in names(chains)) {
    ...
    if (panel==1) {
        legend(-390, log10(6600), c('95% range (PR2011)', '95% range (this work)'), pch=c(15,15), col=c(rgb(.7,.2,.4,.5), rgb(.2,.6,.6,.5)), cex=.8, bty='n')
        ...
        #legend(-380, log10(6500), c('95% credible range','Median','Data'), pch=c(15,NA,4), col=c('gray','black','black'), cex=.8, bty='n')
        #legend(-380, log10(6500), c('95% credible range','Median','Data'), pch=c(NA,'-',NA), col=c('gray','black','black'), cex=.8, bty='n')
    }
