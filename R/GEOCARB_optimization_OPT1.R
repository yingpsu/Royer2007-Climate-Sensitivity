##==============================================================================
## GEOCARB_optimization_OPT1.R
##
## all parameters, to get good estimates of the insensitive (fixed) parameters
##
...
filename.calibinput <- filename.calib_all
...
new_file <- paste(output.dir,'par_deoptim_OPT1_',today,'.rds', sep='')
print(paste('writing file ',new_file, sep=''))
saveRDS(par_deoptim, file=new_file)


## plot against uncalibrated simulation?
if(FALSE){
model_control <- model_forMCMC(par_calib=par_calib0,
...
model_optim <- model_forMCMC(par_calib=par_deoptim,
                             par_fixed=par_fixed0,
                             parnames_calib=parnames_calib,
                             parnames_fixed=parnames_fixed,
                             age=age,
...
plot(-age, model_control, type='l', col='blue', ylim=c(0,10000), lwd=2, xlab='Age (million years)', ylab='CO2 (ppmv)')
for (d in 1:length(data_calib$age)) {points(-data_calib$age[d], data_calib$co2[d], col='red', pch=16)}
lines(-age, model_control, type='l', col='blue', lwd=2)
lines(-age, model_optim, type='l', col='purple', lwd=2)
legend(x=-400, y=8000, c('Control','Optim'), col=c('blue','purple'), lty=c(1,1), lwd=c(2,2), bty='n')
}

# use for mcmc intiial onditions?
#> dim(cov(outDEoptim$member$pop))
#[1] 20 20
